...
struct SimpleSchedule : SigmaSchedule {
    std::vector<float> get_sigmas(uint32_t n, float sigma_min, float sigma_max, t_to_sigma_t t_to_sigma) override {
        ...
        // No steps requested → return empty vector
        if (n == 0) {
            return result_sigmas;
        }

        // Reserve space for n steps plus the final zero sigma
        result_sigmas.reserve(n + 1);

        // Total number of available timesteps (equivalent to len(sigmas) in Python)
        int model_sigmas_len = TIMESTEPS;

        // Factor to map from requested steps (n) to model timesteps
        float step_factor = static_cast<float>(model_sigmas_len) / static_cast<float>(n);

        // Generate sigmas by stepping backwards through the model timesteps
        for (uint32_t i = 0; i < n; ++i) {
            // Position in the model timeline
            int offset_from_start_of_py_array = static_cast<int>(static_cast<float>(i) * step_factor);

            // Convert to reverse index (start from the end)
            ...
            // Convert timestep index to sigma using provided mapping
            result_sigmas.push_back(t_to_sigma(static_cast<float>(timestep_index)));
        }

        // Always append final sigma = 0 (termination)
